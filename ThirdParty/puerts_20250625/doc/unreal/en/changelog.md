...
* Container added UE.BuiltinDouble, fix #1775

* v8 backend supports bytecode

* Support Editor Only Properties by adding _EditorOnly suffix (#1802)

* Support UE 5.5

* v8 backend adds WebSocket support

* Template binding adds support for getters and setters

* UE 5.x adds template binding for FHitResult::GetActor

* Converted FVector and other system USTRUCTs to use template binding to resolve precision loss issues in UE5, fix #1904

* Support properties with only setters

* Support loading blueprints outside the Context directory #1962

#### Optimizations

* Fixed double file reading issue in esm modules, fix #1779

* Added checks for invalid UStruct associations in reflection new, Class.Load, Class.Find, and Class.StaticClass

* Fixed property invalidation when weakfield is valid (#1803)

* Optimized template binding by using FObjectCacheNode's UserData to reduce native C++ map insertions/deletions

* Automatically copy .d.ts files from Plugin directory to Typing directory to avoid maintaining duplicates (#1908)

* Minimized the use of std::string to reduce the probability of issues like https://github.com/Tencent/puerts/issues/1936

* Added abstract markers in TS declarations for native C++ bindings without constructors to trigger errors during TS construction attempts (#1971)

* Added __esModule=false to UE and CPP modules
...
* Restored default Flags for generated blueprint variables to match previous versions (#1814)

* Fixed potential crash during GC of UDynamicDelegateProxy generated by puerts.toDelegate with automatic lifecycle management (#1819)

* Changed SoftObjectPtr to use InstanceTemplate creation to avoid backend-quickjs bug puerts/backend-quickjs#20, fix #1907

* Resolved C2752 error in template binding for const ustruct* matching multiple specializations, fix #1917

* Fixed template binding issue where default values for const SomeClass& parameters read freed stack variables, fix #1924

* Static binding IsInstanceOf now checks if it's an Object first, fix #1929

* Handled memory reuse causing "Check failed: Handle not reset in first callback" error, fix #1930

* Fixed static binding: structs without bound constructors no longer generate constructor definitions (#1951)

* Fixed crash in JSObject copy constructor and assignment overload due to missing HandleScope

* Fixed TCHAR type declaration generation error (#1977)

* Resolved mixin inheritance issue where blueprint component hierarchy wasn't inherited with inherit=true, fix #1985

* Cleaned up WeakMap during unhandledRejection to prevent slow memory release, fix #1991

* Fixed crash when binding Delegates to objects destroyed in the same frame

* Fixed crash in disregardgc scenarios (#1997) fix #1415

* Avoided FName case sensitivity issues in class static members, fix #1882

* Increased quickjs default stack size from 256KB to 1MB to prevent stack overflow (e.g., when running tsc)

* Fixed "NewArray reports t.hasOwnProperty is not a function", fix #2001
